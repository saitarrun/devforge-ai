...
**To update later:**
```bash
sudo npm install -g sdlc-ai-automation@latest
sdlc-ai-automation install
```
...
Or manually check:
```bash
ls ~/.claude/agents/sdlc-ai-automation
```
...
alias update-sdlc-ai-automation='cd ~/path/to/sdlc-ai-automation && git pull origin main && npm install && npm run install-local && make validate'
```

Then simply run:
```bash
update-sdlc-ai-automation
```
...
1. Copies the plugin to `~/.claude/agents/sdlc-ai-automation/` and `~/.claude/commands/sdlc-ai-automation/`
...
```bash
sudo npm uninstall -g sdlc-ai-automation
```
